"""Generate factor_results_3m.md and factor_results_3m.html."""
import json
import sys
...
def generate_markdown(results, out_path):
    ...
    lines = []
    lines.append(f"# A股因子回测结果 — {PERIOD_LABEL}")
    lines.append("")
    lines.append(f"> 回测区间：{PERIOD} | 股票池：15只沪深龙头 | 调仓周期：10日 | Top 5持仓")
    lines.append("")

    lines.append("## 因子绩效汇总")
    lines.append("")
    lines.append("| 因子 | 方向 | 总收益 | Sharpe | 最大回撤 | Calmar | 胜率 | 交易数 |")
    lines.append("|------|------|--------|--------|----------|--------|------|--------|")
    for r in results:
        lines.append(
            f"| {r['factor_name']} | {r['description']} | {r['total_return_pct']}% | "
            f"{r['sharpe_ratio']} | {r['max_drawdown_pct']}% | "
    ...
    lines.append("## 超额收益对比")
    lines.append("")
    header = "| 因子 |" + "|".join(f" {results[0]['benchmarks'][k]['name']} |" for k in bm_keys) + " IR(HS300) |"
    lines.append(header)
...
def generate_html(results, out_path):
    ...
    bm_header = "<th>因子</th>" + "".join(f"<th>vs {bm_names[k]}</th>" for k in bm_keys) + "<th>IR(HS300)</th>"
    bm_rows = ""
    for r in results:
        bm = r["benchmarks"]
        cells = "".join(
...
<title>A股因子回测 — {PERIOD_LABEL}</title>
...
<div class="header">
  <h1>A股因子回测结果 — {PERIOD_LABEL}</h1>
  <p>回测区间：{PERIOD} | 股票池：15只沪深龙头 | 调仓周期：10日 | Top 5持仓 | 初始资金100万</p>
</div>
...
(function() {{
  ...
  const series3 = [{bm_series_js[:-1]}];
  c3.setOption({{
    tooltip: {{ trigger: 'axis' }},
    legend: {{ data: series3.map(s => s.name), bottom: 0, type: 'scroll' }},
...
if __name__ == "__main__":
    json_path = Path(__file__).resolve().parent.parent / "outputs" / "factor_results_3m.json"
    ...
    generate_markdown(results, out_dir / "factor_results_3m.md")
    generate_html(results, out_dir / "factor_results_3m.html")
    print("Done!")

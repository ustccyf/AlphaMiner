"""Backtest each factor individually and collect results for comparison."""
import json
import sys
...
FACTORS = [
    ...
    (VolatilityFactor(20), -1.0, "做多低波动"),  # negative: prefer low vol
    (MADeviationFactor(20), 1.0, "做多偏离均线上方"),
    (PricePositionFactor(20), 1.0, "做多高位"),
    (AmplitudeFactor(20), -1.0, "做多低振幅"),  # negative: prefer low amplitude
...
def run_one(factor, weight, start_dt, end_dt, cash):
    ...
    strategy = MultiFactorStrategy(SYMBOLS, model, data_feed=data_feed, top_n=5, rebalance_days=20)
...
def main():
    start_dt = datetime.strptime("20240101", "%Y%m%d")
    end_dt = datetime.strptime("20241231", "%Y%m%d")
    cash = 1_000_000
    ...
    with open(out_dir / "factor_results.json", "w") as f:
        json.dump(results, f, ensure_ascii=False, indent=2)

    print(f"\nResults saved to {out_dir / 'factor_results.json'}")
